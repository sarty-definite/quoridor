...
exports.canPlaceWall = canPlaceWall;
exports.applyMove = applyMove;
exports.applyPlaceWall = applyPlaceWall;
...
function canPlaceWall(state, wall) {
    ...
    for (const p of state.players) {
        ...
        if (!(0, pathfinding_1.hasPathToGoal)(N, p.pos, goalCheck, walls))
            return false;
    }
    return true;
}
...
function checkWinner(state) {
    ...
    for (const p of state.players) {
        ...
        if (p.startPos) {
            if (p.startPos.r === 0 && p.pos.r === N - 1)
                return p.id;
            if (p.startPos.r === N - 1 && p.pos.r === 0)
                return p.id;
            if (p.startPos.c === 0 && p.pos.c === N - 1)
                return p.id;
            if (p.startPos.c === N - 1 && p.pos.c === 0)
                return p.id;
            continue;
        }
...
function getLegalMoves(state, playerId) {
    ...
    const moves = [];
    const blocked = (0, board_1.getBlockedEdgesFromWalls)(state.walls);
    ...
    for (const d of dirs) {
        ...
        if (!(0, board_1.inBounds)(state.boardSize, n))
            continue;
        // if there's a wall blocking between player and neighbor, skip
        if (blocked.has((0, board_1.edgeKey)(player.pos, n)))
            continue;
        ...
        for (const lat of laterals) {
            ...
            if (blocked.has((0, board_1.edgeKey)(n, diag)))
                continue;

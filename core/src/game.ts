...
export function canPlaceWall(state: GameState, wall: Wall) {
  ...
  if (wall.orientation === 'H') {
    if (wall.r < 0 || wall.r >= N - 1 || wall.c < 0 || wall.c >= N - 1) return false;
  } else {
    if (wall.r < 0 || wall.r >= N - 1 || wall.c < 0 || wall.c >= N - 1) return false;
  }
  ...
  for (const p of state.players) {
    const goalCheck = (pos: Coord) => {
      if (p.goal) return p.goal.axis === 'r' ? pos.r === p.goal.value : pos.c === p.goal.value;
...
export function applyMove(state: GameState, playerId: string, to: Coord): GameState {
  const legal = getLegalMoves(state, playerId);
  if (!legal.find((m) => m.r === to.r && m.c === to.c)) throw new Error('illegal move');
  const next: GameState = { ...state, players: state.players.map((p) => ({ ...p })), walls: [...state.walls] } as any;
...
export function applyPlaceWall(state: GameState, wall: Wall): GameState {
  ...
  if (typeof next.players[idx].wallsRemaining === 'number') {
    if (next.players[idx].wallsRemaining <= 0) throw new Error('no walls remaining');
...
export function getLegalMoves(state: GameState, playerId: string) {
  ...
  const laterals: Coord[] = d.r === 0 ? [{ r: -1, c: 0 }, { r: 1, c: 0 }] : [{ r: 0, c: -1 }, { r: 0, c: 1 }];
